...
def send_signal(symbol, analysis_data, check_result, direction):
    ...
    dir_emoji = '🟢' if direction=='LONG' else '🔴'  # green for LONG, red for SHORT
    ...
    # Multi-line prices under each other, include type
    msg = (
        f"{dir_emoji} {check_result['risk_name']} | {('لانگ' if direction=='LONG' else 'شورت')}\n"
        f"نماد: {clean_symbol}\n"
...
def process_symbol(symbol):
    ...
    for direction in ['LONG','SHORT']:
        ...
        for risk in RISK_LEVELS:
            ...
            print(f"   سطح {risk['emoji']} {risk['name']} → قوانین گذرانده: {res['passed_count']}/9 | دلایل: {', '.join(res['reasons'])}")
            if res['passed']:
