...
def check_rules_for_level(analysis_data, risk_config, direction):
    ...
    # ========== Rule 1: روند 4h (EMA + ساختار) — برای HIGH سخت‌تر ==========
    if '4h' in data and '4h' in closes and len(closes['4h']) >= 55:
        ...
        # برای HIGH حداقل 2 EMA لازم باشه
        min_ema_needed = 2 if risk_key == 'HIGH' else (3 if risk_key == 'LOW' else 2)
    ...
    # ========== Rule 2: روند 1h — برای HIGH سخت‌تر ==========
    if '1h' in data and '1h' in closes and len(closes['1h']) >= 55:
        ...
        struct_count_1h = 3 if risk_key == 'HIGH' else 4
    ...
    # ========== Rule 3: EMA21 + ساختار در 30m — برای HIGH ساختار هم لازم باشه ==========
    ...
    # ========== Rule 4: قدرت کندل 15m — سخت‌تر برای HIGH ==========
    ...
    if '5m' in data and len(data['5m']) >= 10:
        ...
        # آستانه حجم — سخت‌تر برای HIGH
        if risk_key == 'LOW':
            vol_threshold = 1.3
        elif risk_key == 'MEDIUM':
            vol_threshold = 1.2
        else:  # HIGH
            vol_threshold = 1.2  # از 1.1 به 1.2

        bs_break_5m = body_strength(data['5m'][-1]) if len(data['5m']) else 0.0
        effective_mult = 1.1 if (risk_key == 'HIGH' and bs_break_5m >= 0.45) else vol_threshold
    ...
    # ========== Rule 6 & 7: RSI و MACD — برای HIGH هر دو لازم باشن ==========
    ...
    # برای HIGH هر دو RSI و MACD لازم باشن (حداقل 3/5)
    if risk_key == 'HIGH':
        momentum_ok = (rsi_count >= 3) and (macd_count >= 3)
        if momentum_ok:
            passed_rules.append('مومنتوم (RSI + MACD)')
            reasons.append(f"RSI {rsi_count}/5 + MACD {macd_count}/5 همسو")
    else:
        if rsi_ok:
    ...
    # ========== Rule 8: عدم واگرایی — برای همه لازم ==========
    if no_divergence(data, closes):
        passed_rules.append('عدم واگرایی')
        reasons.append("بدون واگرایی در 1h و 4h")

    # ========== Rule 9: حجم اسپایک + قدرت کندل در 15m — سخت‌تر برای HIGH ==========
    if '15m' in data and len(data['15m']) >= 10:
        vol_15m = data['15m'][-1]['v']
        avg_vol_15m = sum(c['v'] for c in data['15m'][-10:]) / 10.0
        vol_multiplier = 1.3 if risk_key == 'LOW' else (1.2 if risk_key == 'MEDIUM' else 1.2)  # HIGH سخت‌تر
        bs15 = body_strength(data['15m'][-1])
    ...
    if risk_key == 'LOW':
        ...
        decision = passed_count >= 6  # از 5 به 6 — حساسیت کمتر
