...
def check_rules_for_level(analysis_data, risk_config, direction):
    ...
    # ========== تصمیم نهایی: آستانه‌ها — اصلاح‌شده و درست ==========
    passed_count = len(passed_rules)
    if risk_key == 'LOW':
        decision = passed_count >= 7      # سخت‌ترین
    elif risk_key == 'MEDIUM':
        decision = passed_count >= 6      # متوسط
    else:  # HIGH
        decision = passed_count >= 5      # آسان‌ترین — ریسک بالاتر، سیگنال بیشتر
